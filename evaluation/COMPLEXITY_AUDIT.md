...
| 2026-08-31 | Claude Opus 5 | v0.6.0 tag, HEAD `1d2ade9` (leads refreshed same day, `3aa43db`) | Cut a release whose binary matches the skills that ship with it, and add a section-discovery warning in the same release — until both land, the shortest path to a novelist's first output does not complete. | Ranked 9 findings + 2 missing items. **The top four trace to no lead at all** — they came from building the binary and running it on a cold copy of `sample_manuscript/` and on a bare `MyNovel.txt`, which is the opposite of the 2026-08-15 run's pattern and the reason this run is worth more. All verified against the code before recording: (1) `v0.6.0` is tagged at `9210b17`, the outliner landed at `73415e2` *after* it, so the shipped `bin/redliner` has no `outline` subcommand while `skills/run/SKILL.md` calls it at assess step 3 — both `plugin.json`s still read `0.6.0` and the bootstrap hook short-circuits on a version match, so no install re-downloads; (2) a directory with no `section_*` files gives `state init` success, `context` `"sections": []` with `"diff": {"verdict": "unchanged"}`, and `validate` exit 0 — the most likely first-run mistake reports all-clear, and `unchanged` is the verdict that tells `recheck` nothing needs re-reading; (3) nothing converts a novelist's one manuscript file into `section_NN.txt` — README's flowchart starts at the layout as a given; (4) `PLATFORMS="darwin/arm64"` only. **Disagreed with a lead:** the author-facing surface is layered, not wide — a novelist types two commands (`/redliner:intake`, `/redliner:run assess`) and never a bare `redliner` subcommand; ~6 concepts, not nine command forms. **Disproved a lead's premise:** the collision-pass entry bundled `canon bundle`/`canon merge` with the pass, but `BundleFacts` reads observation files directly and never opens `collisions.json` or `canon.json` — both subcommands, the joiner, and the id convention all survive dropping the pass. What actually dies is the adjudicator and `likely_unpropagated_revision`, and the joiner cannot inherit the latter because `BundleFacts` emits only `id\|entity\|attribute\|value`. Keep-or-drop is therefore a harder call than TODO.md frames it. Also corrected the 2026-08-15 row's own correction: `cont-5NN` names where numbering *starts*, not a range. **Leads that drew no finding:** python-baseline, the 3× agent-edit cost (it read only the fiction agents), and measurement ceremony — the last two now unanswered twice. Did not run a live `assess`/`line` through real subagents, the same gap as 2026-08-15. Its own least-confident finding (#5: the unbuilt morning edit measured 4/4 while the outliner shipped unmeasured) rests on "no measurement exists," a claim about its search — not independently checked.
