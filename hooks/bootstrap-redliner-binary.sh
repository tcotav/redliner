...
#   writing the real binary directly into the plugin's own bin/
#   directory (confirmed writable at runtime by a live test), so a bare
#   `redliner` command on PATH just works with no wrapper script and no
#   env-var dependency at invocation time.
